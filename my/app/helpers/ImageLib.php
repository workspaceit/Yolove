...
class ImageLib {
	...
	function crop_square_size($source,$width,$height,$type='crop',$newimage=null){
		$this->clear();
		$config['master_dim'] = 'auto';
		$config['source_image'] = $source;
		$config['dynamic_output'] = false;
		...
		$this->initialize($config);
		return $this->resize_choose($width,$height);
	}
	...
	function resize_choose($width,$height){
		...
		if($resize_result){
			...
			if($this->master_dim == 'height'){
				$this->x_axis = ($this->width/2)-($width/2);
				$this->y_axis = 0;
			}elseif ($this->master_dim == 'width'){
				$this->x_axis = 0;
				$this->y_axis = ($this->height/2)-($height/2);
			}
			$this->width = $width;
			$this->height = $height;
	...
	function image_process_gd($action = 'resize')
	{
		...
		//imagefilter($dst_img,IMG_FILTER_SMOOTH,-10.9);
