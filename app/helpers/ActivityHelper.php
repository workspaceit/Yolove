    ...
    public function likeProduct($input, $cuser) {
        ...
        if (!empty($share)) {
            ...
            if ($owner->like_item) {
                $user = User::find($cuser['uid']);
                $emaiData = array();
                $emaiData = array('user' => "<a style='text-decoration:none;color:#46BFC4' href=''>" . $user->nickname . " </a>", 'email' => $owner->email, 'collection' => " on Yolove.it", 'msg' => " Like Your Product ", 'subject' => $user->nickname." Like your Product on Yolove.it ", 'see_post' => "<a style='text-decoration:none;background:#46BFC4;color:#fff;padding:5px 12px;font-weight:700;font-size:14px;border-radius:1px;-moz-border-radius:1px;-webkit-border-radius:1px;' href=''>See Product</a>", 'name' => $owner->nickname);
                $mail = new MailHelper();
    ...
    public function likeAlbum($input, $cuser) {
        ...
        if (!empty($album)) {
            ...
            if ($album->user_id == $cuser['uid']) {
                $album->message = "You Can't Love Your Own Collection";
                return $album;
            }
            $activity = new Activity();
            $activity->user_id = $cuser['uid'];
            ...
            $activity->rel_id = $input['id'];
            $activity->save();
            $activity->message = "You love this collection";
            ...
            if ($owner->like_collection) {
                $user = User::find($cuser['uid']);
                $emaiData = array();
                $emaiData = array('user' => "<a style='text-decoration:none;color:#46BFC4' href=''>" . $user->nickname . " </a>", 'email' => $owner->email, 'collection' => " on Yolove.it", 'msg' => " Like Your Album ", 'subject' => $user->nickname." Like your Album on Yolove.it ", 'see_post' => "<a style='text-decoration:none;background:#46BFC4;color:#fff;padding:5px 12px;font-weight:700;font-size:14px;border-radius:1px;-moz-border-radius:1px;-webkit-border-radius:1px;' href=''>See Album</a>", 'name' => $owner->nickname);
                $mail = new MailHelper();
                $mail = $mail->sendMail($emaiData);
            }
            return $activity;
    ...
    public function unlikeProduct($input, $cuser) {
        ...
        if (!empty($share)) {
            ...
            if (count($islike)) {
                ...
                $islike[0]->message = "You Unlike this Product";
                return $islike[0];
            }
        }
    }
