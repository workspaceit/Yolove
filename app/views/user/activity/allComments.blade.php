...
//        generate('notification', 'center', '<img src="{{ URL::asset("assets/images/LoadingWait.gif")}}">');
        $.ajax({
            url: "<?php echo URL::to('/'); ?>" + "/getNotifications/comments",
            type: "post",
            data: {
            },
            success: function (result) {
                $('.notis').css('display', 'none');
                $('.notis_likes').css('display', 'none');
                $('.notis_comments').css('display', 'block');
                $('.notis_comments').html(result);
            }
        });
    });
