...
<section class="best-sellers">
    <div class="modal-dialog full" style="z-index: 0 !important;">
        ...
        <div class="modal-body clearfix">
            ...
            <div class="arrow-left-control"><a href="javascript:void(0);"><i class="fa fa-chevron-left @if(!$response['status']['islogin']) signup-prompt @else getDetails @endif" id="{{$prevProduct->id}}" data-id="{{$prevProduct->id}}" data-title="{{$prevProduct->slugTitle}}"></i></a></div>
            @endif
            @if(!empty($nextProduct))
            <div class="arrow-right-control"><a href="javascript:void(0);"><i class="fa fa-chevron-right @if(!$response['status']['islogin']) signup-prompt @else getDetails @endif" id="{{$nextProduct->id}}" data-id="{{$nextProduct->id}}" data-title="{{$nextProduct->slugTitle}}"></i></a></div>
            @endif
            <div class="col-md-12 col-sm-12 col-xs-12 popup-img" id="{{$item['id']}}_image">
                ...
                <div class="row set">
                    ...
                    @endif
                    <div class="row">
                        <div class="col-lg-5 col-lg-offset-1 col-sm-5 col-sm-offset-1 col-xs-5 col-xs-offset-1 col-md-5 col-md-offset-1">
                        ...
                        <div class="col-lg-5 col-sm-5 col-xs-5  col-md-5">
                            ...
                            ?>" data-image="{{$item['item']['image_square']}}" data-title="{{$item['item']['title']}}" data-link="{{$item['item']['reference_url']}}" data-shipping-cost="{{$item['shipping_cost']}}">Buy now</button>
                            @endif
                            @else
                            ...
                            @endif
                            <p class="pop-uptext">${{round($item['item']['price'],2)}}</p>
                        </div>
                    </div>
                </div>
                <div class="clearfix"></div>
                                ...
                                <div class="overlay">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 col-xs-6">	
                                            <span>${{round($item->item->price,2)}}</span>
                                        </div>
                                        <div class="col-md-6 col-xs-6">	
                                    ...
                                    <div class="row overlay-btn-container text-center">
                                        ...
                                        <div class="col-lg-5 col-sm-5 col-xs-5 col-md-5">
                                            ...
                                            ?>" data-image="{{$item->item->image_square}}" data-title="{{$item->item->title}}" data-link="{{$item->item->reference_url}}" data-shipping-cost="{{$item->shipping_cost}}" data-toggle="modal" data-target="#addedCart">Buy now</button>
                                            @endif
                                            @else
...
<script>     var image = $('.ssk-group').attr('data-image');
    SocialShareKit.init({
        image: image
    });
    ...
    $('.comment').click(function () {
        ...
        if (cuser != "") {
            ...
            noty({
                ...
                type: 'error',
                dismissQueue: true, timeout: 3000,
                closeWith: ['click'],
                layout: 'center',
                theme: 'defaultTheme',
                maxVisible: 1,
...
//    function getColors() {
//        var colorId = $('#productSize').val();
//        var $selectBox = $('#productColor');
//        var option = '<option value="">Select Color</option>';
//
//        $.ajax({
//            url: "<?php echo URL::to('/'); ?>" + "/getColors/" + colorId,
//            type: "GET",
//            success: function (data) {
//                for (var i = 0; i < data.length; i++) {
//                    option += '<option value="' + data[i].id + '" >' + data[i].color + '</option>';
//                }
//
